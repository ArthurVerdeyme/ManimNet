...
# ────────────────────────────────────────────────────────────
# → “Inputs” at the very top:
FPS = 30                     # Change this to adjust frames per second
SHOW_MESSAGES = False        # Set to False to suppress Manim’s console output
NODE_RADIUS_IMAGE = 0.35      # Radius of each node (image), default is 0.3
NODE_RADIUS = 0.2         # Radius of each node (uniform), default is 0.2
EDGE_OPACITY = 1          # Opacity of all edges (0 to 1), default is 0.6
SHOW_LABELS = False           # Set to False to hide node labels
MATRIX_NODE_RADIUS = 0.12    # smaller radius for the matrix labels, default is 0.12
GRID_BUFF         = 1.0     # how far below the nodes to place the grid, default is 1.0
EDGE_WIDTH       = 2.0     # Width of edges, default is 2.0
...
# Helper function to load a circular image node from group-based folders
def circular_image_node(label, radius=NODE_RADIUS_IMAGE):
    if label == "X":
        path = "sds/verdeyme.png"
    elif label == "Y":
        path = "sds/olhede.png"
    else:
        for folder in ["sds", "biostat", "stat", "mstat", "fam", "fr", "sfam"]:
            candidate = f"{folder}/{label.lower()}.png"
            if os.path.exists(candidate):
                path = candidate
                break
        else:
            path = "picture2.png"
    img = ImageMobject(path).scale_to_fit_height(2 * radius)
    img.set_z_index(1)
    return img

# Preserve original Dot for fallback
OriginalDot = Dot

# Define a new Dot logic: if a label is passed, use image node, else fallback to default Dot
...
class MultiCliqueAnimated7(Scene):
    def construct(self):
        ...
        # Now update dot lists with their white dot equivalents
        def replace_dot_list(dot_list):
            new_list = []
            for dot in dot_list:
        ...
        sds_dots   = replace_dot_list(sds_dots)
        bio_dots   = replace_dot_list(bio_dots)
        stat_dots  = replace_dot_list(stat_dots)
        mstat_dots = replace_dot_list(mstat_dots)
        fam_dots   = replace_dot_list(fam_dots)
        fr_dots    = replace_dot_list(fr_dots)
        sfam_dots  = replace_dot_list(sfam_dots)
